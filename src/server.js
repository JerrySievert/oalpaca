 ...
 * Supports multiple models with LRU memory management and fair request scheduling.
 */
...
class OllamaServer {
  constructor(config, port, host) {
    ...
    this.port = port;
    this.host = host;
    this.model_manager = new ModelManager(config, debug_log);
    this.scheduler = new RequestScheduler(this.model_manager, debug_log);
  }
  ...
  async handle_request(req, res) {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/tags') {
        send_json(res, 200, {
          models: this.model_manager.get_all_model_info()
        });
        return;
      }
      ...
      if (req.method === 'GET' && url.pathname === '/api/ps') {
        send_json(res, 200, {
          models: this.model_manager.get_running_model_info()
        });
        return;
      }
      ...
      if (req.method === 'GET' && url.pathname === '/v1/models') {
        const models = this.model_manager.get_model_names().map((name) => ({
          id: name,
          object: 'model',
          created: Date.now(),
          owned_by: 'local'
